...
## JVM คืออะไร ก่อนจะลงสถาปัตยกรรม

**JVM (Java Virtual Machine)** คือโปรแกรมตัวกลางที่ทำหน้าที่รัน bytecode ของ Java และภาษากลุ่ม JVM อื่น ๆ แทนที่จะให้ code ของเราไปคุยกับระบบปฏิบัติการตรง ๆ JVM จึงเป็นทั้ง:

- runtime ที่จัดการเรื่อง memory, thread, class loading และ garbage collection
- ชั้นแปลกลางที่ทำให้ code เดียวกันไปรันบนหลายระบบปฏิบัติการได้
- ตัว optimizer ที่ตัดสินใจตอนรันจริงว่าจะตีความ, compile, หรือเก็บกวาดอะไรเมื่อไร

ถ้าอยากเห็นภาพแบบไม่เป็นทางการ:

- source code `.java` ไม่ได้วิ่งไปหาเครื่องตรง ๆ
- มันถูกแปลงเป็น **bytecode**
- แล้ว JVM ค่อยเป็นคนรับ bytecode นั้นไปโหลด, แปล, optimize, จัดหน่วยความจำ, และรันให้จริง

เหตุผลที่บทนี้สำคัญคือเวลาแอป Java พัง เราไม่ได้มีแค่คำตอบว่า "โค้ดเราเขียนผิด" แต่มีอีกคำตอบใหญ่คือ "runtime กำลังมีปัญหา" เช่น heap โตผิดปกติ, GC pause ยาว, thread stack กิน memory, หรือ container limit ตึงเกินไป ถ้าไม่รู้ว่า JVM ทำหน้าที่อะไรบ้าง เราจะไล่ปัญหาเหล่านี้ไม่เจอหรือเจอช้ามาก
...
- **Benchmark มือเปล่าโกหกเสมอ** — วัด loop เย็นได้เลขของ interpreter, โค้ดที่ result ไม่ถูกใช้ JIT โยนทิ้งทั้ง loop (dead code) ได้ 0ns — เครื่องมือจริงคือ **JMH (Java Microbenchmark Harness)** ซึ่งบังคับ warm-up + กัน JIT ตัด dead code แทนที่วัดมือเปล่า
- **Deopt storm** — ระบบที่ type/เส้นทางแกว่ง (plugin โหลดเพิ่ม, feature flag สลับถี่) เดิมพัน-ถอย-เดิมพันวน → latency แกว่งลึกลับ — ดูจาก JFR (Java Flight Recorder)
...
- **TLAB (Thread-Local Allocation Buffer)** ชื่อมี thread-local แต่คือ**ช่องส่วนตัวที่กั้นใน Eden ของ heap** (คิวจ่ายเงินแยกช่อง — ของยังวางในห้างเดียวกัน) ไม่ใช่ stack
...
3. **G1 เริ่มก่อนเต็ม**: occupancy แตะ ~45% (IHOP — Initiating Heap Occupancy Percent, เกณฑ์ % ที่ G1 เริ่ม mark ล่วงหน้า) → เริ่ม concurrent marking เบื้องหลังล่วงหน้า — รอเต็มจริงค่อยทำ = full GC ยาวสถานเดียว
