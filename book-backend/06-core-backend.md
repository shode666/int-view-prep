...
JWT ทำงานยังไงโดยย่อ (ต้องเข้าใจก่อนถึงจะเห็นว่าทำไม stateless): token คือ JSON ที่บอกว่าใคร login (`{"sub":"user42","exp":...}`) แล้ว server **เซ็นลายเซ็น**ทับด้วย secret key ของตัวเอง (HMAC) หรือ private key (RSA) พอ client ส่ง token กลับมา server แค่คำนวณลายเซ็นซ้ำด้วย key เดิมแล้วเทียบ — ตรง = token ของแท้ ไม่ถูกแก้ กลไกนี้เองที่ทำให้มัน stateless: การ verify ไม่ต้อง query อะไรเลย ขอแค่ instance มี key เดียวกัน **instance ไหนก็ verify token ที่ instance อื่นออกได้** (จึง scale แนวนอนฟรี) แต่ก็แปลว่า**เพิกถอนกลางคันไม่ได้** — token ที่เซ็นแล้วจะ valid จนถึง `exp` ไม่ว่าจะเกิดอะไรขึ้น เพราะ server ไม่ได้จำว่าออก token ไหนไปบ้าง:
...
- **Cache stampede**: key ยอดนิยมหมดอายุพร้อมกัน → ทุก request ทะลุไป DB พร้อมกัน → DB ตาย ทางกัน: jitter บน TTL (สุ่มบวก/ลบ TTL ไม่กี่วินาที ให้ key หมดอายุคนละจังหวะ ไม่พร้อมกันเป็นก้อน), lock ให้ตัวเดียวไป rebuild (request แรกที่เจอ cache miss คว้า lock — เช่น `SET NX` บน Redis — แล้วไป query DB ตัวเดียว ระหว่างนั้น request อื่นรอหรือ serve ค่าเก่า แทนที่จะทะลุไป DB พร้อมกันทั้งฝูง), serve stale ระหว่าง rebuild
...
token bucket ทำงานยังไง: จินตนาการถังที่จุ token ได้สูงสุด N ใบ (= burst ที่ยอมให้) แล้วมี token หยอดเข้าถังคงที่ r ใบต่อวินาที (= rate เฉลี่ย) ทุก request ต้อง**หยิบ token ออกหนึ่งใบ**ถึงจะผ่าน — ถ้าถังว่าง (token หมด) request นั้นโดนปฏิเสธ (429) — ช่วงที่เงียบ token จะสะสมเต็มถัง พอมี burst มาก็รูดจากที่สะสมไว้ได้ทันที (จึงยอม burst สั้นๆ ได้) แต่พอ token หมดก็ถูกจำกัดที่ r/วินาทีตาม refill — นี่คือเหตุผลที่มัน "burst ได้ในกรอบ rate เฉลี่ย": ขนาดถัง N คุม burst, อัตราหยอด r คุมค่าเฉลี่ยระยะยาว แยกคุมกันได้อิสระ (ต่างจาก fixed window ที่คุมสองอย่างนี้ด้วยตัวเลขเดียว)
