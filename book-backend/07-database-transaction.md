...
- **D — Durability**: commit แล้วไฟดับก็ไม่หาย (WAL — Write-Ahead Log: จดการเปลี่ยนแปลงลง log บน disk **ก่อน**ค่อยแก้ data file จริง — และตอน commit บังคับ fsync คือ syscall ที่สั่ง OS เขียนข้อมูลจาก buffer ลง disk จริงๆ ให้เสร็จก่อนตอบ ok ไม่ค้างใน memory; เหตุที่รอดไฟดับ: การแก้ data file จริงเกิดทีหลังและช้า แต่ถ้าไฟดับกลางคัน ตอน restart DB จะ **replay log** ที่ commit แล้วซ้ำเพื่อกู้สภาพให้ครบ — log ที่ fsync ลง disk แล้วคือหลักฐานที่รอดมา)
...
แปลเป็นภาษาคน: dirty read = เห็นของที่ยังไม่ commit · non-repeatable = อ่านซ้ำในทรานแซกชันเดียวได้ค่าไม่เหมือนเดิม (มีคนแก้แถวเดิม) · phantom = อ่านช่วงเดิมซ้ำได้แถว**เพิ่ม/หาย** (มีคน insert/delete แถวใหม่เข้ามาในช่วงเงื่อนไข เช่น `WHERE amount > 100` ได้ 3 แถวรอบแรก 4 แถวรอบสอง)

ทำไม MySQL/InnoDB กัน phantom ได้ที่ระดับ Repeatable Read (ทั้งที่ตามทฤษฎีต้อง Serializable): มันใช้ **gap lock** — ล็อก "ช่องว่าง" ระหว่างแถวที่ index ครอบ ไม่ใช่แค่แถวที่มีอยู่ ทำให้คนอื่น insert แถวใหม่ลงในช่วงเงื่อนไขที่เรากำลังอ่านไม่ได้จนกว่าเราจะจบ transaction — แถว "ผี" จึงโผล่มาไม่ได้ (Postgres ที่ระดับ Repeatable Read ใช้ snapshot ทั้งก้อนแทน จึงกัน phantom ได้เหมือนกันแต่คนละกลไก)

ทำไม Serializable "ต้อง retry": DB รับประกันว่าผลลัพธ์จะเหมือนกับรัน transaction ทีละตัวเรียงกัน (serial) — พอมันจับได้ว่าสอง transaction ที่รันพร้อมกันจะให้ผลที่ไม่มีลำดับ serial ไหนอธิบายได้ (เช่นต่างคนต่างอ่าน-แล้ว-เขียนทับ) มันจะ **ยกเลิก transaction หนึ่งทิ้งด้วย serialization failure** (Postgres คืน SQLSTATE `40001`) แทนที่จะปล่อยผลผิด — ฝั่ง app จึงต้องดัก error นี้แล้ว **รันทั้ง transaction ใหม่ตั้งแต่ต้น** เอง DB ไม่ retry ให้; ถ้าลืมดัก = transaction นั้นล้มเงียบๆ
...
- ค่าที่ต้องตั้งเสมอ: pool size (ใหญ่ไม่ใช่ดี — Hikari แนะนำราว `cores × 2` ฝั่ง DB), connection timeout (รอ pool ว่าง), idle timeout, **statement timeout** (กัน query ช้ายึด connection)
- **ทำไม pool ใหญ่ไม่ช่วย** (คำถามต่อยอดที่เจอบ่อย): DB ประมวลผล query พร้อมกันได้จริงจำกัดที่จำนวน core/disk — connection ที่เกินนั้นไม่ได้ทำงานขนานเพิ่ม แค่ไปแย่ง CPU กันจนต้อง context-switch ถี่ๆ + แย่ง lock/memory มากขึ้น = throughput ตกและ latency แกว่ง (pool 500 บน DB 8 core ช้ากว่า pool 20 ด้วยซ้ำ) หลักคือ pool ควรเท่า concurrency ที่ DB *รับไหว* ไม่ใช่เท่าจำนวน request ที่วิ่งเข้ามา — request ส่วนเกินรอในคิว pool ดีกว่าไปทับถม DB
