...
## คำเหล่านี้คืออะไรก่อน

บทนี้เปิดมาด้วยคำที่หลายคนเคยได้ยิน แต่ไม่ได้แปลว่าทุกคนเห็นภาพตรงกัน จึงปูความหมายสั้น ๆ ก่อน:

- **DevOps** ในบริบทของบทนี้ไม่ได้หมายถึงตำแหน่งงานหนึ่งตำแหน่ง แต่หมายถึงชุดความรับผิดชอบและวิธีทำงานที่ทำให้ทีมพัฒนา "ส่งของขึ้น production ได้บ่อย ปลอดภัย และรู้เร็วเมื่อพัง"
- **CI (Continuous Integration)** = ทุกครั้งที่ merge code ต้องผ่านด่านอัตโนมัติ เช่น build, test, lint, security scan
- **CD (Continuous Delivery/Deployment)** = ทำให้การเอาของขึ้น production เป็นงาน routine ไม่ใช่งานพิธีกรรมเสี่ยงสูง
- **SRE (Site Reliability Engineering)** = สายงานที่โฟกัส reliability, automation, observability และ operational excellence ของระบบ
- **Observability** = ความสามารถในการมองเห็นอาการภายในของระบบจากสิ่งที่มันปล่อยออกมา เช่น logs, metrics, traces
- **OWASP** = องค์กรและชุดความรู้ด้าน web security ที่คนใช้เป็นภาษากลางเวลาอธิบายช่องโหว่ยอดฮิต
- **Kubernetes (K8s)** = ระบบจัดการ container จำนวนมากให้ deploy, scale และ recover ได้อัตโนมัติมากขึ้น

ถ้าจะย่อให้เหลือประโยคเดียว บทนี้กำลังตอบคำถามว่า: **"หลังจากเราเขียนระบบเสร็จแล้ว เราจะพามันขึ้นจริง ดูแลมัน และกันมันพังหรือโดนเจาะอย่างมีวินัยได้ยังไง"**
