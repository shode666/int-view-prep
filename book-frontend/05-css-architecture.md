...
| Performance | CSS เพียว เร็วสุด | CSS เพียว + code-split ตาม component | ไฟล์ CSS เดียวเล็กมาก (utility ซ้ำใช้ร่วม) | จ่าย runtime ทุก render; ขัดกับ React Server Components (RSC)/streaming (บท 6) |
...
(โค้ดข้างล่างใช้ `oklch(L C H)` — color function สมัยใหม่ที่ระบุสีด้วย lightness/chroma/hue: `L` = ความสว่าง 0–1, `C` = ความอิ่มสี, `H` = องศาเฉดสี เลือกใช้แทน hex/rgb เพราะปรับความสว่างแล้วสีไม่เพี้ยน เหมาะกับงาน token/theme ที่ต้อง generate เฉดอ่อน-เข้มอย่างสม่ำเสมอ — Baseline ทุก browser หลักแล้ว จะเขียนเป็น hex ก็ได้ ไม่กระทบหลักการสามชั้น)
