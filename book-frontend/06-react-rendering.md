...
`'use client'` ที่หัวไฟล์**ไม่ได้แปลว่า "รันเฉพาะ client"** — มันคือป้ายประกาศ **จุดตัดของ bundle**: จากไฟล์นี้ลงไป (รวมทุกอย่างที่มัน import) เป็นโลก client ที่ต้องส่ง JavaScript ไปให้ browser ค่า props ที่ข้ามพรมแดน server → client ต้อง **serialize ได้** (serialize = แปลงค่าในหน่วยความจำเป็นข้อความ/ไบต์ที่ส่งข้ามเครือข่ายได้ แล้วฝั่งรับ deserialize กลับ — จำเป็นเพราะ server กับ browser เป็นคนละ process คนละเครื่อง ส่ง reference ตรงๆ ไม่ได้) รูปแบบที่ผ่านคือ JSON + ชนิดพิเศษที่ React รองรับ เช่น promise — ส่ง function, class instance, Date ที่ต้อง method ครบ ข้ามไปไม่ได้ เพราะพฤติกรรม (โค้ดของ method) แปลงเป็นข้อความแล้วประกอบกลับไม่ได้ (ข้อยกเว้นเดียวคือ Server Action ที่ประกาศ `'use server'`) ท่าสำคัญที่คนพลาด: server component **ส่งเป็น children เข้า client component ได้** — ดังนั้น "โลก client" ไม่จำเป็นต้องลามทั้ง subtree
