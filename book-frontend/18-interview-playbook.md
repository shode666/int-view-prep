...
ช่วง "มีอะไรถามเราไหม" คือรอบสัมภาษณ์ที่เขากำลังถูกสัมภาษณ์กลับ — คำถามที่ดีโชว์ว่าคุณคิดแบบไหนโดยไม่ต้องพูดถึงตัวเอง:

| # | คำถาม | ทำไมฉลาด |
|---|---|---|
| 1 | "Frontend deploy บ่อยแค่ไหน แล้ว rollback ครั้งล่าสุดเกิดอะไรขึ้น" | วัด maturity ของ CI/CD จริงจากเหตุการณ์จริง ไม่ใช่คำโฆษณา |
| 2 | "มี performance budget หรือ CWV target ไหม ใครเป็นเจ้าของตัวเลข" | โชว์ว่าคิดเรื่อง performance เป็นระบบ + เผยว่าที่นี่มีเจ้าภาพหรือไม่มีใครดู |
| 3 | "Design system อยู่ stage ไหน ทีม product ปฏิบัติกับมันยังไง" | คำตอบบอกทั้ง tech maturity และการเมืองระหว่างทีมในคำถามเดียว |
| 4 | "เทสหน้าตาเป็นไง ratio ประมาณไหน แล้วทีมเชื่อ test suite แค่ไหน" | คำว่า "เชื่อ" คือกับดัก — ที่ที่ test flaky คนจะหน้าเปลี่ยนทันที |
| 5 | "Tech debt ฝั่ง FE ก้อนใหญ่สุดตอนนี้คืออะไร มีแผนจัดการยังไง" | ทุกที่มี debt — ที่วัดคือเขา*รู้*และ*มีแผน*ไหม และคุณจะได้รู้ว่ากำลังเซ็นรับอะไร |
| 6 | "Junior ถึง senior ที่นี่ต่างกันตรงไหน มีเส้นทางโตยังไง" | โชว์ว่ามองยาว + วัดว่าที่นี่นิยาม senior ด้วยอะไร (ปีหรือ impact) |
| 7 | "Incident ฝั่ง FE ล่าสุดคืออะไร แล้วหลังจากนั้นเปลี่ยนอะไรไปบ้าง" | วัดวัฒนธรรม postmortem — ที่ที่ตอบว่า "ไม่ค่อยมี incident" คือธงแดง |
| 8 | "ทีม FE มีเสียงในการออกแบบ API ไหม หรือรับ contract ที่ backend กำหนดมา" | คำถามที่คนเคยเจ็บจาก contract แย่ๆ เท่านั้นจะถาม — และเผย power dynamic ของทีม |
| 9 | "การตัดสินใจ technical ใหญ่ๆ เกิดยังไง — ใครตัดสิน ด้วยข้อมูลอะไร" | วัดว่า engineer มีส่วนร่วมหรือรอรับคำสั่ง — ตัวชี้วัดความสุขระยะยาว |
| 10 | "อะไรทำให้คนที่เคยทำตำแหน่งนี้ไม่รอด" | คำถามกลับด้านที่ได้คำตอบจริงเสมอ — เขาต้องนึกถึงคนจริงที่ล้มเหลวจริง |
