...
เรื่อง extends vs `&` ในแถวสุดท้ายต่างกันตรง "เจอ field ชื่อซ้ำแต่ type ขัดกันแล้วเกิดอะไร": `interface B extends A` ที่ประกาศ field ชื่อเดียวกับ `A` เป็นคนละ type จะ **error ตรงจุดประกาศทันที** ("incompatibly") — คนแก้รู้เลยว่าผิดตรงไหน ส่วน `type C = A & B` (intersection = "ต้องเป็นทั้ง A และ B พร้อมกัน") ไม่ error ตอนประกาศ แต่ TS พยายาม intersect type ของ field ที่ชนกัน เช่น `{ id: string } & { id: number }` → `id: string & number` ซึ่งไม่มีค่าไหนเป็นได้ทั้ง string และ number พร้อมกัน จึงยุบเหลือ `never` **เงียบ ๆ** — โค้ดยังคอมไพล์ผ่าน แต่ไปพังตอนพยายาม assign ค่าจริงให้ `id` ทีหลัง (error ไกลจากต้นเหตุ)
...
2. numeric enum รับ "เลขอะไรก็ได้" ในบางเวอร์ชันเก่าและมี reverse mapping ที่คนงง, `const enum` (enum ที่ TS พยายาม inline ค่าแทนการ generate object) มีปัญหากับ `isolatedModules` — โหมด/flag ที่บังคับให้ทุกไฟล์คอมไพล์ได้แยกกันทีละไฟล์โดยไม่เห็นไฟล์อื่น (bundler สมัยใหม่อย่าง esbuild/Vite ต้องการโหมดนี้เพื่อความเร็ว) แต่ inline ค่า const enum จำเป็นต้องอ่านนิยาม enum จากอีกไฟล์ — ขัดกับกติกา "แต่ละไฟล์ต้องแปลได้ด้วยตัวเอง" มันจึงพังหรือถูกแบนใน pipeline พวกนี้
