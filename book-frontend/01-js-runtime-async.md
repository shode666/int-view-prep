...
flattening คือกลไกที่ทำให้ chain ไม่กลายเป็น Promise ซ้อน Promise: ปกติ callback ที่ return object `X` จะทำให้ promise ใหม่ fulfilled ด้วย `X` ทันที — แต่ถ้า `X` เป็น promise (มี `.then` เป็น function เรียกว่า *thenable*) spec สั่งพิเศษว่าอย่าเอา promise ตัวนั้นมาเป็น "ค่า" แต่ให้ promise ใหม่ไป **subscribe รอผล** ของ `X` แล้วค่อย settle ตามผลนั้น ผลคือ `.then` ไม่มีวันคืน `Promise<Promise<T>>` — มันถูกแบนเหลือชั้นเดียวเสมอ (นี่คือเหตุผลว่าทำไม `await` ตัวเดียวพอสำหรับ `res.json()` ที่ตัวมันเองคืน promise อยู่แล้ว)
...
- `await` **หยุดเฉพาะฟังก์ชันตัวเอง** ไม่หยุด main thread — เครื่องมือจริงข้างใต้คือ generator + state machine ("generator" = ฟังก์ชันชนิดที่ **หยุดกลางคันแล้วกลับมาทำต่อได้** ไม่เหมือนฟังก์ชันปกติที่รันรวดเดียวจนจบ — มันจำได้ว่าค้างไว้ที่บรรทัดไหนและตัวแปรมีค่าอะไร): compiler มอง `async function` เป็นเครื่องจักรที่มี "สถานะ = ค้างอยู่ตรง await ตัวที่เท่าไร" เจอ `await` ฟังก์ชันจึง *suspend* — บันทึกตำแหน่งบรรทัดปัจจุบันกับค่าตัวแปรทั้งหมดไว้ในสถานะนั้น แล้วคืน control ให้ caller ทันที พอ promise settle โค้ดส่วนที่เหลือ (นับจากบรรทัดหลัง `await`) ถูกนัดกลับมารันต่อจากสถานะที่ค้างไว้ ในรูป **microtask**
